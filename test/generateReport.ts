...
async function printReport() {
    ...
    if (process.env.DISCORD_WEBHOOK) {
        ...
        await fetch(process.env.DISCORD_WEBHOOK, {
            ...
            body: JSON.stringify({
                description: "Here's the latest bluecord Report!",
                username: "bluecord Reporter",
...
page.on("console", async e => {
    ...
    const isVencord = (await args[0]?.jsonValue()) === "[bluecord]";
...
function runTime(token: string) {
    ...
    try {
        ...
        bluecord.Util.Logger.prototype._log = function (level, levelColor, args) {
            if (level === "warn" || level === "error")
                console[level]("[bluecord]", this.name + ":", ...args);
        };

        // force enable all plugins and patches
        bluecord.Plugins.patches.length = 0;
        Object.values(bluecord.Plugins.plugins).forEach(p => {
            p.required = true;
            p.patches?.forEach(patch => {
                ...
                bluecord.Plugins.patches.push(patch);
            });
        });

        bluecord.Webpack.waitFor(
            "loginToken",
            m => {
        ...
        bluecord.Webpack.onceReady.then(() => setTimeout(async () => {
            console.error("[PUP_DEBUG]", "Webpack is ready!");

            const { wreq } = bluecord.Webpack;
            ...
            for (const patch of bluecord.Plugins.patches) {
                if (!patch.all) {
                    new bluecord.Util.Logger("WebpackInterceptor").warn(`Patch by ${patch.plugin} found no module (Module id is -): ${patch.find}`);
                }
            }
