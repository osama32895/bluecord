...
export async function importSettings(data: string) {
    ...
    if ("settings" in parsed && "quickCss" in parsed) {
        ...
        throw new Error("Invalid Settings. Is this even a bluecord Settings file?");
}
...
export async function downloadSettingsBackup() {
    const filename = "bluecord-settings-backup.json";
...
export async function uploadSettingsBackup(showToast = true): Promise<void> {
    if (IS_WEB) {
        ...
        const [file] = await DiscordNative.fileManager.openFiles({
            filters: [
                { name: "bluecord Settings Backup", extensions: ["json"] },
                { name: "all", extensions: ["*"] }
            ]
        });
