...
console.log("[bluecord] Starting up...");
...
if (!process.argv.includes("--vanilla")) {
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "bluecord");

    electron.app.whenReady().then(() => {
        // Source Maps! Maybe there's a better way but since the renderer is executed
        // from a string I don't think any other form of sourcemaps would work
        electron.protocol.registerFileProtocol("bluecord", ({ url: unsafeUrl }, cb) => {
            let url = unsafeUrl.slice("bluecord://".length);
        ...
        try {
            ...
            if (settings.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[bluecord] Installed React Developer Tools"))
                    .catch(err => console.error("[bluecord] Failed to install React Developer Tools", err));
        } catch { }
    ...
    console.log("[bluecord] Running in vanilla mode. Not loading bluecord");
}

console.log("[bluecord] Loading original Discord app.asar");
// Legacy bluecord Injector requires "../app.asar". However, because we
...
if (readFileSync(injectorPath, "utf-8").includes('require("../app.asar")')) {
    console.warn("[bluecord] [--> WARNING <--] You have a legacy bluecord install. Please reinject");
